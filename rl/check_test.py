...
class Tests(unittest.TestCase):
    ...
    def mean_score_check(self, scores):
        assert (len(scores) >= 100 and np.mean(list(map(itemgetter('score'), scores[-100:]))) >= 195.0)
...
def run_check(check_name, func):
    try:
        ...
        printmd('**TEST RESULT: <span style="color: red;">PLEASE TRY AGAIN</span>**')
        return
    printmd('**TEST RESULT: <span style="color: green;">PASSED</span>**')